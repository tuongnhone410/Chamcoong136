...
/**
 * ConfigurableCalculationEngine calculates attendance and salary dynamically
 * based on WorkRule, OvertimeRule, and ShiftEntity configurations without hardcoded constants.
 */
class ConfigurableCalculationEngine(
    private var defaultWorkRule: WorkRule? = null,
    private var defaultOvertimeRule: OvertimeRule? = null,
    ...
    private fun getMillisForTime(baseTimeMs: Long, timeStr: String, dayOffset: Int = 0): Long {
        ...
        val hour = parts.getOrNull(0)?.toInt() ?: 0
    ...
    override fun calculateSingleEntry(entry: TimeEntry, config: UserConfig?): TimeEntry {
        ...
        val normInMs = if (rawIn <= stdInMs) stdInMs else rawIn

        val isOvernightShift = shift.shiftType == "NIGHT" || com.example.data.model.ShiftEntity.isOvernight(shift.startTime, shift.endTime)
        val dayOffset = if (isOvernightShift) 1 else 0

        val normOutMs = if (rawOut != null) {
            val stdOutMs = getMillisForTime(rawIn, shift.endTime, dayOffset)
            if (rawOut >= stdOutMs) {
                val diffMs = rawOut - stdOutMs
                if (diffMs < 30 * 60000L) stdOutMs else rawOut
            } else {
                rawOut
            }
        } else {
        ...
        val standardHours = workingEntry.snapshotStandardHours ?: defaultWorkRule?.standardHoursPerDay ?: shift.standardHours

        val lateMin = if (normInMs > stdInMs) ((normInMs - stdInMs) / 60000.0).toInt() else 0
        val earlyLeaveMin = if (normOutMs != null) {
            val stdOutMs = getMillisForTime(rawIn, shift.endTime, dayOffset)
            if (normOutMs < stdOutMs) ((stdOutMs - normOutMs) / 60000.0).toInt() else 0
        } else {
            0
        }
        ...
        val workD = if (rawOut == null) {
            ...
            when {
                actualWorkedHrs >= standardHours -> 1.0
                actualWorkedHrs >= standardHours / 2.0 -> 0.5
        ...
        val otStartAfter = defaultWorkRule?.overtimeStartAfterHours ?: standardHours
        val otHrs = if (normOutMs != null) {
            val workedHrs = (normOutMs - normInMs) / 3600000.0
            val actualWorkedHrs = (workedHrs - breakHrsToUse).coerceAtLeast(0.0)
            (actualWorkedHrs - otStartAfter).coerceAtLeast(0.0)
        } else {
            0.0
        }
    ...
    override fun calculateAllowanceValue(
        ...
        return when (calcType) {
            ...
            "PER_WORK_DAY" -> {
                if (fieldName == "pcComCa") comCaCount * allowanceValue else totalWorkDays * allowanceValue
            }
    ...
    override fun calculateMonthlySalary(
        ...
        val expectedDays = if (totalScheduledDaysInMonth > 0) totalScheduledDaysInMonth else 26
        val stdHours = defaultWorkRule?.standardHoursPerDay ?: 8.0
        val dailySalary = luongBasic / expectedDays.toDouble()
        val hourlySalary = dailySalary / stdHours
        ...
        var totalStandardHours = unworkedHolidaysCount * stdHours
        ...
        val normalOtMult = defaultOvertimeRule?.normalDayMultiplier ?: config.heSoOtNgayThuong
        val sundayOtMult = defaultOvertimeRule?.weeklyOffMultiplier ?: config.heSoOtChuNhat
        val holidayOtMult = defaultOvertimeRule?.holidayMultiplier ?: config.heSoOtNgayLe
        val nightOtMult = config.heSoOtDem

        for (e in processedEntries) {
            if (isCurrentSelectedMonth && e.date > todayStr && e.rawCheckIn == null) continue
            val isHolidayDateVal = holidayDatesInMonth.contains(e.date)
            if (isHolidayDateVal && e.rawCheckIn == null) continue
            ...
            if (e.dayType == "UNPAID_LEAVE" || e.dayType == "UNAUTHORIZED_LEAVE") continue
            if (e.rawCheckIn == null) continue

            if (e.shiftType == "NIGHT") nightShiftsCount++
            ...
            if (e.rawCheckOut == null && e.isWorking) {
                if (isSundayVal) {
                    ...
                    totalSundayHours += stdHours
                    sundayPay += stdHours * hourlySalary * sundayOtMult
                } else {
            ...
            if (isSundayVal) {
                ...
                sundayPay += actualHours * hourlySalary * sundayOtMult
            } else {
                totalWorkDays += e.workDay
                actualPresenceDaysCount++
                ...
                val workedHrs = (e.normalizedCheckOut!! - e.normalizedCheckIn!!) / 3600000.0
                val actualHours = (workedHrs - eBreakHours).coerceAtLeast(0.0)
                val finalStandardHours = actualHours.coerceAtMost(stdHours)
                totalStandardHours += finalStandardHours

                val finalOtHours = e.otHours
                if (finalOtHours > 0.0) {
                    val otMult = e.snapshotOtMultiplier ?: when {
                        e.dayType == "HOLIDAY" -> holidayOtMult
                        isSundayVal -> sundayOtMult
                        else -> normalOtMult
                    }
                    if (e.dayType == "HOLIDAY") {
                        totalOtLeHours += finalOtHours
                        otLePay += finalOtHours * (hourlySalary * holidayOtMult)
                    } else if (e.shiftType == "NIGHT") {
                        totalOtNightHours += finalOtHours
                        otNightPay += finalOtHours * (hourlySalary * nightOtMult)
                    } else {
                        totalOtDayHours += finalOtHours
                        otDayPay += finalOtHours * (hourlySalary * otMult)
                    }
                }
            }
        }
        ...
        for (e in processedEntries) {
            if (isCurrentSelectedMonth && e.date > todayStr && e.rawCheckIn == null) continue
            if (e.rawCheckIn == null) continue
            if (e.workDay >= 1.0) comCaCount++
            if (e.otHours >= 1.0) comOtCount++
        }
        ...
        val chuyenCanValue = if (hasUnpaidOrAbsent) 0.0 else calculateAllowanceValue(
            "tienChuyenCanGoc",
