...
(.*?)
    ...
    \) \{
        item \{
            Text\("5\. & 6\. Phụ cấp & Tiền cơm", fontSize = 16\.sp, fontWeight = FontWeight\.Bold, color = White\)
            Text\("Các khoản phụ cấp cố định, tiền cơm ca và tiền cơm làm thêm giờ\.", fontSize = 12\.sp, color = LightGray\)
                ...
                onClick = \{
                    if \(validate\(\)\) \{
                        val updated = userConfig\.copy\(
                            ...
                            pcComTC = pcComTc\.toDoubleOrNull\(\) \?: userConfig\.pcComTC
                        \)
...
\1
    ...
    val hasUnsavedChanges = pcXangXe != userConfig.pcXangXe.toString() ||
        ...
        pcComTc != userConfig.pcComTC.toString()
